...
// Handle trainer profile picture using the function
$profile_pic = getProfilePicture($trainer_data['profile_picture']);
...
while ($row = $result_pending->fetch_assoc()) {
    // Add profile picture path to each request
    $row['user_profile_pic'] = getProfilePicture($row['profile_picture']);
...
while ($row = $result_active->fetch_assoc()) {
    // Add profile picture path to each client
    $row['user_profile_pic'] = getProfilePicture($row['profile_picture']);
    $active_clients[] = $row;
}
$stmt_active->close();
...
while ($row = $result->fetch_assoc()) {
    // Add profile picture path to each session
    $row['user_profile_pic'] = getProfilePicture($row['profile_picture']);
    $sessions[] = $row;
}
$stmt->close();
